...
//   const { handleSetBookContent } = useContext(ModalContext);
  ...
  return (
   <div>
      <div className="flex flex-col h-[290px] items-center">
        {
          book.volumeInfo.imageLinks ?
          <img className=" w-[180px]" src={book.volumeInfo.imageLinks?.thumbnail} alt={book.volumeInfo.title} />
          :
      ...
      <div className="flex flex-col gap-1 items-center">
        ...
        <p className="text-sm text-dark-blue uppercase">
            {book.volumeInfo.authors ? <p>{book.volumeInfo?.authors[0]}</p> : <p>Desconhecido</p>}
        </p>
      </div>
    </div>
  );
